...
export function ManualCode({ onValidate, onBack }: { onValidate: () => void; onBack: () => void }) {
  ...
  return (
    <div className="bg-white relative size-full overflow-hidden" data-name="iPhone 13 & 14 - 33">
      ...
      {/* Center content */}
      <div className="absolute flex flex-col items-center left-[29px] top-[calc(30%+16px)] w-[332px]">
        <BarcodeIcon />
        <p className="font-['Host_Grotesk:ExtraBold',sans-serif] font-extrabold leading-[34px] text-[#2c2c2c] text-[30px] text-center mt-[24px]">Ingresa el codigo de tus zapatillas</p>
      </div>

      {/* Input + button */}
      <div className="absolute flex flex-col gap-[16px] items-start left-[29px] top-[calc(50%+92px)] w-[332px]">
        <div className="flex flex-col gap-[8px] items-start w-full">
          <div className="flex gap-[4px] items-center">
            <p className="font-['Host_Grotesk:SemiBold',sans-serif] font-semibold leading-[18px] text-[#111] text-[14px]">Codigo de barra</p>
        ...
        >
          <span className={`font-['Host_Grotesk:SemiBold',sans-serif] font-semibold leading-[18px] text-[14px] px-[4px] whitespace-nowrap ${valid ? "text-white" : "text-white/80"}`}>Validar codigo</span>
        </PressableButton>
      </div>
    </div>
  );
